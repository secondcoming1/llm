...
###############
def evaluate_model_on_sequences(model, original_sequence, counterfactual_sequence):
    """
    Evaluates the model's reconstruction error on the original anomalous session 
    and the generated counterfactual session.

    Parameters:
    - model: Trained LSTM autoencoder model.
    - original_sequence: The original anomalous session (numpy array, shape=(1, timesteps, features)).
    - counterfactual_sequence: The generated counterfactual session (numpy array, shape=(1, timesteps, features)).

    Returns:
    - Dict containing reconstruction errors for both original and counterfactual sessions.
    """
    
    # Get model predictions (reconstructions)
    original_reconstruction = model.predict(original_sequence)
    counterfactual_reconstruction = model.predict(counterfactual_sequence)

    # Compute reconstruction error (Mean Absolute Error)
    original_error = np.mean(np.abs(original_sequence - original_reconstruction))
    counterfactual_error = np.mean(np.abs(counterfactual_sequence - counterfactual_reconstruction))

    print("\n🔍 **Model Evaluation Results:**")
    print(f"🛑 Original Anomalous Session Reconstruction Error: {original_error:.6f}")
    print(f"✅ Counterfactual Session Reconstruction Error: {counterfactual_error:.6f}")

    # Compare results
    if counterfactual_error < original_error:
        print("✔️ Counterfactual successfully reduces the anomaly score! 🎯")
    else:
        print("❌ Counterfactual did not significantly reduce anomaly score. Needs adjustment.")
...
def main():
    ...
    counterfactual_array = counterfactual_example.to_numpy()
    ...
    evaluation_results = evaluate_model_on_sequences(lstm_model, anomalous_sequence, counterfactual_array)
